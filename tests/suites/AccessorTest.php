...
class AccessorTest extends TestCase
{
    ...
    public function testTransformers(): void
    {
        ...
        $accessor = new Accessor('.', $transformer);
        $subject = [
            'string-val' => 'A string',
            'null-val' => null,
            'assoc-array-val' => [
        ...
        $result = $accessor->get($subject, 'string-val');
        $this->assertEquals('A string', $result);
        $result = $accessor->get($subject, 'null-val', coerce: Type::STRING);
        ...
        $result = $accessor->get($subject, 'object-val.non-existing', 'The default');
        $this->assertEquals('The default', $result);
        $result = $accessor->get($subject, 'object-val.null-val-3', coerce: Type::STRING);
        $this->assertEquals('null', $result);
        $result = $accessor->get(null, 'non-existing', 'The default');
        $this->assertEquals('The default', $result);
    }
